...
Sub evalTbl(Optional tbln = "check", Optional dp As debugPrint = debugPrint.faNone)
...
Function mkTest(tbl, Optional dp As debugPrint = debugPrint.faNone)
    Call evalTbl(tbl, dp)
...
Function fnAryToStr(fnAry0)
    Dim ret, tmp
    Dim fn
    fn = getAryAt(fnAry0, 1)
    tmp = mapA("fnAryElmToStr", dropAry(fnAry0, 1))
    If fn = "id_" Then
        ret = getAryAt(tmp, 1)
    ElseIf fn = "l_" Then
...
Function mkStatement(fnAry0, vz, retIsObj, Optional withAssert, Optional expected, Optional dp As debugPrint = debugPrint.faNone)
    Dim ret
    If vz = "" Then
        Select Case dp
            Case faNone: ret = ""
            Case faDebugPrint: ret = "Debug.Print " & fnAryToStr(fnAry0)
            Case faPrintAry: ret = "printAry " & fnAryToStr(fnAry0)
            Case Else
        End Select
    ElseIf retIsObj Then
        ret = "Set " & vz & " = " & fnAryToStr(fnAry0)
    Else
        ret = vz & " = " & fnAryToStr(fnAry0)
    End If
    If withAssert Then
...
Sub mkCheckTbl(tbln)
    ...
    y = ActiveCell.Offset(0, 2).Address(False, False)
    z = ActiveCell.Offset(1, 0).Resize(1, n).Address(False, False)
    Call addBtn(x, "'evalTbl(""" & tbln & """)'", "eval")
    Call addBtn(y, "'clearActual(""" & tbln & """)'", "clear")
    Range(z) = ary
...
Sub testcheck()
    ...
    x = mkSeq(6, -5, 3)
    y = mapA("calc", x, 5, "*")
    'assert y, Array(-25, -10, 5, 20, 35, 50)
    pl = Array(2, 0, 3, 1, 0)
    str1 = polyStr(pl)
    ...
    z = pi_quarter * 4
    root2 = Sqr(2)
    assert Round(root2, 5), 1.41421
    Set dic = mkDic("a", 1, "b", 2)
    Set tmp = dic
    Set clc = mkClc(1, 2, 3, sp)
    Set tmp = clc
    'assert tmp, mkClc(1, 2, 3, sp)
End Sub
